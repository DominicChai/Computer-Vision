   ...
   for opt, arg in opts:
      if opt == '-h':
         ...
         outputfile = arg
          ...
          size=arg
   count4(inputfile,outputfile,size)



def count4(inputfile,outputfile,n):
    n=int(n)
    ...
    for column in range(numOfRows): #column and numOfRows can not be the same name
        for row in range(numOfColumns):
            if img[column,row,0] == 255:
                ...
                if (levelMat[column-1,row]==0 and levelMat[column,row-1]==0) :
                        #print("no neighbors with labels")
                        levelMat[column,row]=label
                        dictionary[label]=set()
                        label=label+1
                else:
                    a=[levelMat[column-1,row],levelMat[column,row-1]]
                    try:
                        a.remove(0)
                    except:
                        pass
